...
model = Model.model(os.path.join(os.getenv("MODEL_PATH"), "uecfood256.keras"))
...
@app.get("/metadata")
async def metadata():
    file = open(os.path.join("metadata", "uecfood256.json"), 'rb')
    content = file.read()
    file.close()
...
async def predict(file: UploadFile = File(...), top_k: int = 10):
    ...
    metadata = json.loads(open(os.path.join("metadata", "uecfood256.json"), 'r', encoding="utf-8").read())["data"]
